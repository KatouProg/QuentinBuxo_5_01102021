...
function getCart() {
  ...
  // --> Le panier est plein
    ...
    localStorageArticle.forEach((article) => {
      ...
      productDelete.setAttribute(
        "onclick",
        `deleteProduct("${article.articleID}","${article.articleColor}")`
      );
    });
  }
}
...
function getTotal() {
  ...
  for (let i = 0; i < myLength; ++i) {
    totalPrice +=
      articleQuantity[i].valueAsNumber * localStorageArticle[i].articlePrice;
  }
  ...
  for (let k = 0; k < quantityMod.length; k++) {
    quantityMod[k].addEventListener("change", (event) => {
      ...
      const resultFind = localStorageArticle.find(
        (el) => el.quantityModValue !== quantityModif
      );
...
function deleteProduct(articleID, color) {
  localStorageArticle = localStorageArticle.filter(
    (el) => el.articleID !== articleID || el.articleColor !== color
  );
  localStorage.setItem("produit", JSON.stringify(localStorageArticle));
